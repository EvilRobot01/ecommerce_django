...
def remove_from_cart(request, pk):
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            messages.info(request, 'This item was no in your cart')
            return redirect('App_Shop:home')
    else:
...
def increase_cart(request, pk):
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            messages.info(request, 'You do not have as active order')
            return redirect('App_Shop:home')
    ...
    if order_qs.exists():
        ...
        if order.orderitems.filter(item=item).exists():
            ...
            if order_item.quantity >= 1:
                ...
                order.orderitems.remove(order_item)
                order_item.delete()
                messages.info(request, f'{item.name} has ben removed from your cart')
                return redirect('App_Order:cart')
        else:
            messages.info(request, 'You do not have as active order')
            return redirect('App_Shop:home')